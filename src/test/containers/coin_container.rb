...
class CoinContainer
  def create(options = {})
    ...
    bash_cmd += "./doublesevencoind " + cmd_args.join(" ")

    if options[:remove_addr_after_shutdown]
      bash_cmd += "; rm -f /root/.doublesevencoin/testnet/peers.dat"
    end

    if options[:remove_wallet_after_shutdown]
      bash_cmd += "; rm /root/.doublesevencoin/testnet/wallet.dat"
    end
