...
/** PoW Money limit **/
static const int64 LAST_POW_MONEY = 1500000000 * COIN;
...
static const int COINBASE_MATURITY_DSC = 500;
/** Threshold for nLockTime: below this value it is interpreted as block number, otherwise as UNIX timestamp. */
static const int STAKE_TARGET_SPACING = 10 * 60; // 10-minute block spacing
static const int STAKE_MIN_AGE = 60 * 60 * 24 * 3; // minimum age for coin age 3day
...
static const uint256 hashGenesisBlockOfficial("0x00000003c6ad03efc24389584570c454cffe1f80999dad730de5c3ffd61b74d3");
static const uint256 hashGenesisBlockTestNet("0x00000005029ba7a3b0e1fac7e005bec5b45ff11fbe770386bac3d04dde025610");
...
public:
    ...
    {
        // dscoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // dscoin: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // dscoin: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // dscoin: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // dscoin: whether transaction is a coinstake
    bool fCoinStake;

    // dscoin: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // dscoin: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // dscoin
            undo.nTime = nTime;            // dscoin
        }
        return true;
    }
...
public:
    ...
    // dscoin: block signature - signed by coin base txout[0]'s owner
    ...
    // dscoin: two types of block: proof-of-work or proof-of-stake
    ...
    // dscoin: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // dscoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // dscoin: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (dscoin proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // dscoin: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // dscoin: proof-of-stake related block index fields
    unsigned int nFlags;  // dscoin: block index flags
    enum
    {
