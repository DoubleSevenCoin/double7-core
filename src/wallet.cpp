...
// dscoin: optional setting to unlock wallet for block minting only;
...
int64 CWalletTx::GetTxTime() const
{
    ...
    //dscoin: we still have the timestamp, so use it to avoid confusion
    return nTime;
}
...
// dscoin: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->nTime > nSpendTime)
                continue;  // dscoin: timestamp must not exceed spend time
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            ploop
            {
                ...
                // dscoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else if (!GetBoolArg("-avatar")) // dscoin: not avatar mode; nu: avatar mode enabled by default only on Share wallet to avoid change being sent to hidden address
                    {
...
// dscoin: create coin stake transaction
...
// dscoin: check 'spent' consistency between wallet and txindex
// dscoin: fix wallet spent state according to txindex
...
// dscoin: disable transaction (only for coinstake)
