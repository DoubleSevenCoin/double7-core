...
// Copyright (c) 2011-2018 The DoubleSevencoin developers
...
int nCoinbaseMaturity = COINBASE_MATURITY_DSC;
...
const string strMessageMagic = "DoubleSevencoin Signed Message:\n";
...
void SyncWithWallets(const uint256 &hash, const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // dscoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        // dscoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // dscoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
{
    if (pindexBest == NULL || pindexBest->nMoneySupply < 50000000 * COIN)
        return 10000 * COIN;
    if (pindexBest->nMoneySupply < (50000000 + 50000000) * COIN)
        return 1000 * COIN;
    if (pindexBest->nMoneySupply < (400000000 + 50000000 + 50000000) * COIN)
        return 0.1 * COIN;
...
// dscoin: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// dscoin: find last block index up to pindex
...
#endif
{
    ...
    // dscoin: target change every block
    // dscoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
    ...
    else if(pindexBest->nMoneySupply > (50000000) * COIN){
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // dscoin: should not enter safe mode for longer invalid chain
}
...
bool CTransaction::CheckInputs(CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks) const
{
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // dscoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // dscoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // dscoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool *pfClean)
{
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (undo.nHeight != 0) {
                    ...
                    coins.fCoinStake = undo.fCoinStake; // dscoin
                    coins.nTime = undo.nTime;           // dscoin
                } else {
                    if (coins.IsPruned())
    ...
    // dscoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // dscoin: coinbase reward check relocated to CheckBlock()
    ...
    // dscoin: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // dscoin: fees are not collected by miners as in bitcoin
    // dscoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// dscoin: total coin age spent in transaction, in the unit of coin-days.
...
// dscoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(CValidationState &state, const CDiskBlockPos &pos)
{
    ...
    // dscoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // dscoin: record proof-of-stake hash value
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // dscoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return state.DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // dscoin: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // dscoin: check that the block satisfies synchronized checkpoint
    ...
    // dscoin: check pending sync-checkpoint
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    // dscoin: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
    ...
    // dscoin: ask for pending sync-checkpoint if any
    ...
    if (pblock->hashPrevBlock != 0 && !mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // dscoin: getblocks may not obtain the ancestor block rejected
    ...
    // dscoin: if responsible for sync-checkpoint send it
...
// dscoin: entropy bit for stake modifier if chosen by modifier
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(int, CBlockIndex*)& item, vSortedByHeight)
    {
        ...
        // dscoin: calculate stake modifier checksum
    ...
    // dscoin: load hashSyncCheckpoint
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "DoubleSevencoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
bool InitBlockIndex() {
    ...
    fTxIndex = true; // dscoin: txindex is always enabled
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "21-MAY-2018 Behind New Yorks Housing Crisis Weak Laws and Uneven Regulation";
        CTransaction txNew;
        txNew.nTime = 1526885302;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1526885302;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 829161504;

        if (fTestNet)
        {
            block.nTime    = 1526898756;
            block.nNonce   = 643149243;
        }
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0xd3265d42934e81006ef68ffb85e2c2584fead465b67c5bbdf0012a5795ebbcbf"));
        block.print();
        assert(hash == hashGenesisBlock);
        // dscoin: check genesis block
        {
            CValidationState state;
        ...
        // dscoin: initialize synchronized checkpoint
        if (!WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // dscoin: if checkpoint master key changed must reset sync-checkpoint
...
string GetWarnings(string strFor)
{
    ...
    // dscoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // dscoin: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // dscoin: safe mode for high alert
            }
        }
    }
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            // dscoin: relay memory may contain blocks too
            bool found = false;
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // dscoin: send latest proof-of-work block to allow the
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // dscoin: record my external IP reported by peer
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // dscoin: tell downloading node about the latest block if it's
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // dscoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // dscoin: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("DoubleSevencoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("DoubleSevencoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DoubleSevencoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DoubleSevencoinMiner : ProcessBlock, block not accepted");
    }
...
#endif
{
    ...
    RenameThread(fProofOfStake? "dscoin-stake-minter" : "dscoin-miner");
    ...
    try { ploop {
        ...
        if (fProofOfStake)
        {
            // dscoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
        ...
        printf("Running DoubleSevencoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("DoubleSevencoinMiner terminated\n");
        throw;
    }
}
...
{
    // dscoin: mint proof-of-stake blocks in the background
